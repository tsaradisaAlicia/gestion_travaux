...
  // ✅ CORRECTION : Les fonctions fetch sont déplacées à l'intérieur du useEffect
  // pour résoudre l'erreur de d\u00E9pendance.
  useEffect(() => {
...
      // ✅ CORRECTION : Relancer la fonction de chargement des utilisateurs.
      // Note : Comme fetchUtilisateurs est maintenant dans le useEffect,
      // vous pouvez cr\u00E9er une version locale \u00E0 l'int\u00E9rieur de cette fonction
      // ou utiliser un \u00E9tat pour d\u00E9clencher le rechargement.
      // Pour la d\u00E9mo, nous allons le faire manuellement.
      const fetchUtilisateursLocal = async () => {
        const canView = userRole === 'Admin' || userRole === 'RRH' || userRole === 'Assistante DES DIRECTIONS';
        if (!canView) {
          setUtilisateurs([]);
          return;
        }
        const configLocal = getAuthConfig();
        if (!configLocal) {
            alert("Vous devez être connecté pour voir cette page.");
            setUtilisateurs([]);
            return;
        }
        try {
          const response = await axios.get('http://localhost:5000/api/users', configLocal);
          setUtilisateurs(response.data);
        } catch (error) {
          console.error('Erreur lors du chargement des utilisateurs:', error.response ? error.response.data : error.message);
        }
      };
      fetchUtilisateursLocal();
    } catch (error) {
...
      // ✅ CORRECTION : Recharger la fonction de chargement des utilisateurs.
      const fetchUtilisateursLocal = async () => {
        const canView = userRole === 'Admin' || userRole === 'RRH' || userRole === 'Assistante DES DIRECTIONS';
        if (!canView) {
          setUtilisateurs([]);
...
        return;
    }
    // ✅ AVERTISSEMENT : window.confirm est d\u00E9conseill\u00E9. Remplacez par un modal UI.
